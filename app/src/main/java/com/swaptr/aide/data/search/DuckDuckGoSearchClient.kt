...
// Calls are blocking; tool framework already invokes on worker thread.
@Singleton
class DuckDuckGoSearchClient @Inject constructor(
    ...
    fun search(query: String, max: Int = 3): List<Hit> {
    ...
    // Legacy String API; prefer fetchOutcome (distinguishes empty body from HTTP error).
    fun fetch(url: String, maxChars: Int = 2500): String =
        when (val out = fetchOutcome(url, maxChars)) {
            is FetchOutcome.Text -> out.text
            is FetchOutcome.Empty -> ""
            is FetchOutcome.Error -> throw IOException(out.message)
        }
    ...
    fun fetchOutcome(url: String, maxChars: Int): FetchOutcome {
        ...
        return try {
            http.newCall(req).execute().use { resp ->
                if (!resp.isSuccessful) {
                    val bucket = if (resp.code in 400..499) "HTTP_4XX" else "HTTP_5XX"
                    return FetchOutcome.Error(bucket, "HTTP ${resp.code}")
                }
            ...
            FetchOutcome.Error("NETWORK", e.message ?: e.javaClass.simpleName)
        }
    }
...
// Separate from downloader's client so connection-pool state and timeouts stay isolated.
@javax.inject.Qualifier
@Retention(AnnotationRetention.BINARY)
annotation class SearchHttp
