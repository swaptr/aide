...
class TaskStripView(
    ...
    fun start(scope: CoroutineScope) {
        stop()
        chipsJob = scope.launch { chipsFlow.collect { rebuild(it) } }
    ...
    private fun taskChip(task: TaskEntity): AideButton = aideButton(
        ctx = ctx,
        shape = AideButtonShape.Pill,
