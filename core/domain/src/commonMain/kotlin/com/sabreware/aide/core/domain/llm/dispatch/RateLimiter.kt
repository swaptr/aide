...
class RateLimiter {
    ...
    private val categoryFor: Map<String, Category> = buildMap {
        ...
        listOf(
            ...
            "SetFlashlight", "ShowLocationOnMap",
    ...
    fun resetTurn(turnId: String) = synchronized(lock) {
    ...
    fun acquire(toolName: String, turnId: String, cap: Int): Outcome = synchronized(lock) {
        ...
        if (current >= cap) {
            ...
            return@synchronized Outcome.Denied(retryInSeconds = 0)
        }
    ...
    fun forget(turnId: String) {
        synchronized(lock) { perTurn.remove(turnId) }
    }
}
