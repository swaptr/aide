...
object SystemPromptBuilder {
    ...
    /** Heading for tools that belong to no toolset (MCP, the meta-tool). */
    private const val OTHER = "Other"

    fun build(
        surface: Surface,
        tools: List<AideTool>,
        /** Contribution order of the registered toolsets — the order categories are advertised in. */
        categoryOrder: List<String>,
        webSearchEnabled: Boolean,
        onDemandCategories: List<String> = emptyList(),
        /** Extra guidance each active toolset asked to add (granted filesystem roots, and so on). */
        toolsetNotes: List<String> = emptyList(),
        /** Tools this surface cannot reach — used for the "switch to chat" suggestion, not for filtering. */
        chatOnlyToolNames: List<String> = emptyList(),
    ): String {
        val functions = tools.filterIsInstance<AideTool.Function>()
        // On-demand categories live behind RequestToolset — advertising full schemas here
        // would negate the wire-size savings of the engine-side filter.
        val eagerFunctions = functions.filter { !it.requiresActivation }
        // Each tool carries the category of the toolset that produced it, so there is no name table to
        // keep in step with the tools themselves.
        val grouped = eagerFunctions.groupBy { it.category ?: OTHER }
        ...
        lazyToolsetBlock(onDemandCategories)?.let { out.append("\n").append(it) }
        surfaceNotes(surface, chatOnlyToolNames)?.let { out.append("\n").append(it) }
        toolsetNotes.forEach { out.append("\n").append(it) }
    ...
    private fun surfaceNotes(surface: Surface, chatOnlyToolNames: List<String>): String? = when (surface) {
        ...
        Surface.IME -> {
            // The tools were already filtered out before the model saw them; this list exists so the
            // assistant can tell the user WHY, and it is derived from the tools' own declared surfaces
            // rather than from a hand-kept list that drifts.
            val unavailable = chatOnlyToolNames.sorted().joinToString(", ")
            if (unavailable.isEmpty()) null else "Surface notes (keyboard):\n" +
    ...
    private val ERROR_MEANINGS: Map<String, String> = mapOf(
        ...
        "NO_FLASH_UNIT" to "this device has no camera flash to use as a torch",
        "TORCH_FAILED" to "the camera HAL rejected the torch change (often: camera in use)",
        "LAUNCH_FAILED" to "system rejected the intent dispatch",
        "NO_HANDLER" to "no installed app handles the requested intent",
