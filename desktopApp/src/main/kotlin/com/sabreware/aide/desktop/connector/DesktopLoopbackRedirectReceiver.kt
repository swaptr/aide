...
/**
 * Desktop peer of `:app`'s `LoopbackRedirectReceiver` (same logic; `ServerSocket` is JVM-only so it can't be
 * commonMain). Binds a transient `ServerSocket` on `127.0.0.1:<random>`, serves one "you can close this tab"
 * page, and feeds the captured code/state into the shared [PendingOAuthFlow]. The gate is armed and the
 * blocking `accept()` is launched at construction (before the browser opens), so a redirect can never race
 * ahead of arming; on `accept()` the flow is marked receiving so a concurrent resume-cancel can't drop a
 * completing sign-in. Loopback-only (backlog 1, not reachable off-device); always closed on exit.
 */
class DesktopLoopbackRedirectReceiver(
