...
class AideButton(ctx: Context) : FrameLayout(ctx) {
    ...
    // Reused so the touch host can linger the pressed highlight a few ms past release — a clearly
    // visible flash on fast taps (mirrors LatinIME's delayed key-preview dismiss) — with no
    // per-keypress allocation.
    val unpressAction = Runnable { isPressed = false }

    private var pillShape: Boolean = false

    // Keys swap a pressed colour instead of rippling — a manually-driven ripple double-flashes.
    internal var flatPressed: Boolean = false
    ...
    fun applyStyle(ctx: Context, style: AideButtonStyle, shape: AideButtonShape = AideButtonShape.Rounded()) {
        ...
        background = buildBackground(ctx, shape, resolved, flatPressed)
        label?.let {
            it.setTextColor(resolved.fg)
            // Modifier-ish styles bold their label; Standard stays normal.
            val bold = style !is AideButtonStyle.Standard && style !is AideButtonStyle.Borderless
...
// `onTap` is the base output: fired on UP for normal keys, on DOWN for repeatable keys (which then
// auto-repeat via `onRepeat`). `alts`/`popup`/`onAltChosen` drive the long-press more-keys popup.
class Key(
    val view: AideButton,
    ...
    val isRepeatable: Boolean get() = onRepeat != null
    val isLongPressEnabled: Boolean get() = alts != null
}
...
sealed class AideButtonStyle {
    ...
    /** Borderless key: no resting box, a key-shaped highlight appears only on press (Gboard-style). */
    object Borderless : AideButtonStyle()
...
internal class ResolvedStyle(
    ...
    // When set, the key is transparent at rest and fills with this colour on press (borderless).
    val pressedBg: Int? = null,
)
...
internal fun AideButtonStyle.resolve(ctx: Context): ResolvedStyle {
    ...
    return when (this) {
        ...
        AideButtonStyle.Borderless -> ResolvedStyle(
            bg = ColorStateList.valueOf(0x00000000),
            fg = ContextCompat.getColor(ctx, R.color.aide_on_surface),
            ripple = csl(R.color.aide_outline_variant),
            pressedBg = ContextCompat.getColor(ctx, R.color.aide_surface_container_high),
        )
...
fun aideButton(
    ...
    val flatPressed = touchMode == AideButtonTouchMode.DispatchedByParent
    val container = AideButton(ctx).apply {
        this.flatPressed = flatPressed
        background = buildBackground(ctx, shape, resolved, flatPressed)
    ...
    if (label == null) {
        ...
        val tv = AppCompatTextView(ctx).apply {
            ...
            setPadding(padH, 0, padH, 0)
            val isBold = style !is AideButtonStyle.Standard && style !is AideButtonStyle.Borderless
...
private fun buildBackground(
    ...
    fun roundRect(color: Int) = GradientDrawable().apply {
        cornerRadius = radiusPx
        setColor(color)
    }
    val resting = roundRect(style.bg.defaultColor)

    // Keys: instant pressed-colour swap, no ripple (borderless has its own; filled keys derive one).
    if (flatPressed) {
        val pressedColor = style.pressedBg ?: pressedTint(style.bg.defaultColor, style.fg)
        return StateListDrawable().apply {
            addState(intArrayOf(android.R.attr.state_pressed), roundRect(pressedColor))
            addState(StateSet.WILD_CARD, resting)
        }
    }

    // Native buttons keep the ripple (framework-coordinated, no double).
    val fill: Drawable = if (style.pressedBg != null) {
        StateListDrawable().apply {
            addState(intArrayOf(android.R.attr.state_pressed), roundRect(style.pressedBg))
            addState(StateSet.WILD_CARD, resting)
        }
    } else {
        resting
    }
    val mask = roundRect(0xFFFFFFFF.toInt())
    return RippleDrawable(style.ripple, fill, mask)
}

/** Material "pressed" state layer: [fg] over [bg] at ~20% — one clear, flat highlight for a key. */
private fun pressedTint(bg: Int, fg: Int): Int {
    val a = 0.20f
