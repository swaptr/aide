...
object SpeechAssetCatalog {
    ...
    /**
     * Assets the app must fetch alongside [spec] but never asks the user to pick — VAD is
     * infrastructure, not a choice, so it is absent from every picker and rides along with the first
     * model that needs it.
     *
     * Only NON-streaming ASR needs it: those decode a whole utterance in one shot and have no idea
     * where it ended, so Silero supplies the endpoint. Streaming ASR (Zipformer) carries its own
     * endpointer, and TTS never listens.
     */
    fun companionsFor(spec: SpeechAssetSpec): List<SpeechAssetSpec> =
        if (spec.modality == Modality.Asr && !spec.family.streaming) listOf(sileroVad) else emptyList()
    ...
    fun ofKind(kind: Modality): List<SpeechAssetSpec> = when (kind) {
        Modality.Asr -> sttAssets
        Modality.Tts -> ttsAssets
        Modality.Vad -> vadAssets
        else -> emptyList()
    }
